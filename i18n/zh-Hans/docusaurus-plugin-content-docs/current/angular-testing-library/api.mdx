...
一个用于设置组件的 `@Input` 和 `@Output` 属性的对象。
...
更多信息见[Angular文档](https://angular.io/api/core/NgModule#declarations)。
...
用不同的props重新渲染同一个组件。没有定义的输入属性被清除。这在props更新后调用 `detectChanges`。
