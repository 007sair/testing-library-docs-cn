...
请查看[ThePacielloGroup的 "什么是可访问的名称？"](https://developer.paciellogroup.com/blog/2017/04/what-is-an-accessible-name/)。
...
例如 `<img aria-label="fancy image" src="fancy.jpg" />` 对于 `getByAltText('fancy image')` 和
 `getByRole('img', { name: 'fancy image' })` 都将被返回。
