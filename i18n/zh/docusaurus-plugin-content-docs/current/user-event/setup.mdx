...
When users interact in the browser by e.g. pressing keyboard keys, they interact with a UI layer the browser shows to them. The browser then interprets this input, possibly alters the underlying DOM accordingly and dispatches [trusted](https://developer.mozilla.org/en-US/docs/Web/API/Event/isTrusted) events.  
...
The `userEvent.setup()` API applies these workarounds to the document and allows you to [configure](options.mdx) an "instance" of `user-event`.  
...
The [Clipboard API](https://developer.mozilla.org/en-US/docs/Web/API/Clipboard) is usually not available outside of secure context.  
To enable testing of workflows involving the clipboard, [`userEvent.setup()`](setup.mdx) replaces `window.navigator.clipboard` with a stub.
