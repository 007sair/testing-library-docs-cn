...
[`jasmine-dom`][gh] is a companion library for Testing Library that provides custom DOM element matchers for Jasmine

```bash npm2yarn
npm install --save-dev @testing-library/jasmine-dom
```

Then follow [usage section][gh-usage] from jasmine-dom's documentation to add the matchers to Jasmine.
...
Check out [jasmine-dom's documentation][gh] for a full list of available matchers.
