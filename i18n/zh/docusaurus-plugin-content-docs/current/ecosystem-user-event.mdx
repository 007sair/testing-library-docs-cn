...
[`user-event`][gh] is a companion library for Testing Library that provides more advanced simulation of browser interactions than the built-in [`fireEvent`](dom-testing-library/api-events.mdx#fireevent) method.

:::caution End of life

This page describes `user-event@13.5.0`.  
This version is no longer maintained. Please use [`user-event@14`](user-event/intro.mdx) instead, as it includes important bug fixes and new features.

:::

## Installation<Tabs defaultValue="npm" values={[{ label: 'npm', value: 'npm' }, { label: 'Yarn', value: 'yarn' }]}> <TabItem value="npm"> ```sh npm install --save-dev @testing-library/user-event @testing-library/dom ``` </TabItem> <TabItem value="yarn"> ```sh yarn add --dev @testing-library/user-event @testing-library/dom ``` </TabItem> </Tabs>
